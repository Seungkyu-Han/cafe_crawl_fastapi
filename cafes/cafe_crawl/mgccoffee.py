...
def get_category_value() -> list[tuple[int, str]]:
    res = requests.get(f'{BASE_URL}/menu/?menu_category1=1&menu_category2=1', headers=headers)
...
def make_menu_url(category: int, page: int) -> str:
    return f'{BASE_URL}/menu/menu.php?page={page}&category={category}&menu_category1=1&menu_category2=1'

def crawl_mgc_menus() -> CafeCrawlRes:
    menu_categories: list[MenuCategory] = []
    category_value_map: list[tuple[int, str]] = get_category_value()
    ...
    for category_value, category_name in category_value_map:
        ...
        for cur_page in range(1, 100):
            res = requests.get(make_menu_url(category=category_value, page=cur_page), headers=headers)
        ...
        menu_categories.append(MenuCategory(category=Category(name=category_name, order=category_value), menus=menus))
